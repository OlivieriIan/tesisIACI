...
\underline{\textbf{Entradas}}: Señal de DMX, Fin de carrera, Dip-switch, Encoder de disco, Encoder de motor.\\
\underline{\textbf{Salidas}}: Freno, Motor, Led interno (led indicador de la Placa de control), Led externo (led indicador del Dipswitch).

\newpage
\section{Justificación del proyecto}
Durante el desarrollo del updown la empresa se dió cuenta que con los recursos que contaban, tiempo en particular, no podían concretar los requerimientos que buscaban que el updown tuviera. Estos requerimientos incluyen: desarrollo de un dipswitch para el direccionamiento de DMX, programación del microcontrolador para manejar todo el sistema, modificaciones mecánicas necesarias para mejorar el desempeño del equipo y software de prueba para la calibración de los equipos, entre otros.\\

Por este motivo, Blackout se vió en la necesidad de buscar a alguien externo a la empresa para darle fin al proyecto, y eentonces que se presentó la oportunidad de realizar el trabajo de concluir el desarrollo del equipo, que es en lo que este proyecto final se basa.

\section{Objetivos}
Para dar conclusión al proyecto se deben cumplir con las requerimientos no cumplidos del proyecto. Esto incluye:
\begin{itemize}
	\item Lograr que las cargas manejadas por los equipos se muevan a la velocidad y posición indicadas, con el objetivo de que los efectos espaciales creados sean visualmente creibles. Estas referencias de posición y velocidad serán dadas por una consola DMX.
	...
	\item Realizar los cambios mecánicos y electrónicos necesarios en el equipo para que tenga las funcionalidades deseadas. Esto incluye desarrollo del dipswitch, y modificaciones de hardware que se necesiten.
	\item Generar documentación y programas de prueba para que facilitar la calibración y el armado en cantidad de los updown.
